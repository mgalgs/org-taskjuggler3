...
(defcustom org-export-taskjuggler-project-tag "taskjuggler_project"
  "Tag, property or todo used to find the tree containing the project definition."
  :group 'org-export-taskjuggler
  :type 'string)

(defcustom org-export-taskjuggler-task-tag "taskjuggler_task"
...
(defcustom org-export-taskjuggler-globals-tag "taskjuggler_globals"
  "Tag, property or todo used to find the tree containing all the
global declarations for the project."
  :group 'org-export-taskjuggler
  :type 'string)

(defcustom org-export-taskjuggler-source-code-drawer "TJ"
  "The name of the drawer that contains taskjuggler source code.
  You can add this drawer to any node, and its contents will be
  exported directly to taskjuggler in the context of that node."
  :group 'org-export-taskjuggler
  :type 'string)
...
The exporter first finds a heading with tag
`org-export-taskjuggler-project-tag', which it exports as the
project definition. The exporter then looks for a tree with tag,
property or todo that matches `org-export-taskjuggler-task-tag'
and takes this as the tasks for this project. The first node of
this tree defines the project properties such as project name and
project period. If there is a tree with tag, property or todo
that matches `org-export-taskjuggler-resource-tag' this three is
taken as resources for the project. If no resources are
specified, a default resource is created and allocated to the
project. Also the taskjuggler project will be created with
default reports as defined in
`org-export-taskjuggler-default-reports'."
  ...
  (let* ((projects 
	...
	(org-taskjuggler-fixup-project-list-with-good-defaults projects)
	(org-taskjuggler-fixup-resources-list-with-good-defaults resources)
	(org-taskjuggler-fixup-tasks-list-with-good-defaults tasks)

    (with-current-buffer buffer
      (erase-buffer)
      (org-taskjuggler-open-project (car projects))
      ...
      (org-taskjuggler-export-reports)
      (save-buffer)
...
export."
  (let (flags
	...
	(dolist (pair org-todo-key-alist)
	  (unless (symbolp (car pair))
		(add-to-list 'flags (car pair))))
	flags))

(defun org-taskjuggler-fixup-project-list-with-good-defaults (projects)
  "Add a default version to the project if none was given, and
add a start date if none was given."
  (let ((project (car projects)) 
		version)
	(unless (assoc "version" project)
	  (setq version org-export-taskjuggler-default-project-version))
	(setcar projects (push (cons "version" version) project))
	(unless (assoc "start" project)
	  (setcar projects (push (cons "start" (format-time-string "%Y-%m-%d")) project)))))

(defun org-taskjuggler-fixup-resources-list-with-good-defaults (resources)
  "Add a default resource if none are specified."
  (unless resources
	(setq resources 
...
(defun org-taskjuggler-fixup-tasks-list-with-good-defaults (tasks)
  "Add a default allocation and start date to the first task if
none were given."
...
information, all the properties, taskjuggler source code stored
in the `org-export-taskjuggler-source-code-drawer' drawer, etc."
  (let* ((props (org-entry-properties))
	...
	(push (cons "source-code" (org-taskjuggler-get-source-block)) props)))
...
(defun org-taskjuggler-get-source-block (&optional point)
  "Return the contents of the
`org-export-taskjuggler-source-code-drawer' drawer, which
...
attributes from the PROJECT alist are inserted. If no end date or duration is
specified, the duration is calculated
`org-export-taskjuggler-default-project-duration' days from now."
  (let* ((unique-id (cdr (assoc "unique-id" project)))
		 ...
		 (duration (or (cdr (assoc "duration" project)) org-export-taskjuggler-default-project-duration))
		 (end (cdr (assoc "finish" project))))
    (insert (format "project %s \"%s\" \"%s\" %s" unique-id headline version start))
	(if end
		(insert (format " - %s {\n" end))
		(insert (format " +%sd {\n" duration)))
	(insert 
  ...
  (let ((id (org-taskjuggler-clean-id 
    ...
    (insert 
     (concat 
	  ...
	  (and state (format " flags %s\n" state))
	  (cdr (assoc "source-code" resource))))))
...
(defun org-taskjuggler-export-reports ()
  "Export the reports section to the current buffer."
  (let (report)
